...
rows[1..-1].each do |row|
    ...
    obj = {
        ...
        knnpbi: columns[7].text.strip.to_f
    }
...
standings[80..800].select{|x| my_results_map.key?(x[:name])}.sample(175).each{|x|
    my_result = my_results_map[x[:name]]
    battles = (if my_result[:knnpbi] < 7 then 6 else 8 end)
    puts "new Pair<>(\"#{x[:name]}\", #{battles}),"
}
