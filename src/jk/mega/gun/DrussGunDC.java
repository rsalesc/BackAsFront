...
public class DrussGunDC extends Component implements BulletListener, ScannedRobotListener, PaintListener {
   final static boolean TC = true;
   ...
   ScannedRobotEvent lastScan;
    ...
    Point2D.Double futurePoint = new Point2D.Double(); // DUMMY POINT SO AS TO DO NOT TAKE MIRROR MOVEMENT INTO ACCOUNT
...
//      }
      ...
      lastDirection = direction; // TODO: report bug
   ...
   //             myNextLocation,BULLET_POWER,direction);
      ...
      Controller controller = bot.getGunControllerOrDummy();
//        AdvancedRobot controller = bot;
      ...
      if( ((TC && bot.getEnergy() != 0.0) || bot.getEnergy() > wave.bulletPower)
       & maxEscapeAngle/(Math.PI/9 - Math.PI/18) + 0.99> bot.getGunHeat()/bot.getGunCoolingRate() & e.getEnergy() > 0){
         ...
         controller.setTurnGunRightRadians(FastTrig.normalRelativeAngle(
            - bot.getGunHeadingRadians() + mostVisitedAngle));
         if(Math.abs(bot.getGunTurnRemainingRadians()) < Math.abs(18/e.getDistance()))
            gunAimed = true;
         controller.setTurnGunRightRadians(FastTrig.normalRelativeAngle(-bot.getGunHeadingRadians() + mostVisitedAngle));
      	
      }
      else
         controller.setTurnGunRightRadians(FastTrig.normalRelativeAngle(nextAbsBearing - bot.getGunHeadingRadians()));
      ...
      if(rs.MEA_pos != 0.0 & rs.MEA_neg != 0.0){
         ...
         //  )
            if(gunFinishedTurning  && waveList.size() > 1)
               if(dataExists)
                  b = controller.setFireBullet(wave.bulletPower);
...
class DCWave  {
   ...
   DCRobotState scan;
   private RobotMediator robot;
   private double distanceTraveled;

   DCWave(RobotMediator _robot) {
      this.robot = _robot;
   }
