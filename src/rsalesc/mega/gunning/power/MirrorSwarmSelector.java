...
import rsalesc.baf2.tracking.EnemyFireEvent;
import rsalesc.baf2.tracking.EnemyFireListener;
import rsalesc.mega.utils.StatData;

/**
 * Created by Roberto Sales on 20/09/17.
 */
public class MirrorSwarmSelector extends Component implements PowerSelector, EnemyFireListener {
    MirrorPowerSelector duelSelector = new MirrorPowerSelector();
    MeleePowerSelector meleeSelector = new MeleePowerSelector();

    @Override
    public void init(RobotMediator mediator) {
        super.init(mediator);
        duelSelector.init(mediator);
    }
