...
import rsalesc.mega.utils.MatrixUtils;
...
public class MLP {
    ...
    private MLPStrategy     outputStrategy;
    ...
    public MLP setOutputStrategy(MLPStrategy strategy) {
    ...
    public MLPStrategy getOutputStrategy() {
    ...
    public MLPStrategy getLayerStrategy(int i) {
        if(i == 0)
            throw new IllegalStateException();

        return i == layersSizes.length - 1 ? outputStrategy : strategy;
    }
    ...
    public double[][] feed(double[][] batch) {
        ...
        for(int i = 0; i < layersSizes.length - 1; i++) {
            ...
            for(int j = 0; j < batchSize; j++) {
                ...
                double[] a = getLayerStrategy(i + 1).getActivation(MatrixUtils.getColumn(next, j));
                for(int k = 0; k < layersSizes[i + 1]; k++)
                    next[k][j] = a[k];
            }
    ...
    private void backPropagate(int hiddenLayer, double[][] prop, double rate, boolean lastLayer) {
        double[][] error = lastLayer ? getLayerStrategy(hiddenLayer + 1).getLastLayerGradient(act[hiddenLayer + 1], prop) : prop;

        if(hiddenLayer > 0)
            backPropagate(hiddenLayer - 1,
                    getLayerStrategy(hiddenLayer).getNextLayerGradient(act[hiddenLayer], multiply(transpose(weights[hiddenLayer]), error)),
                    rate, false);
    ...
    public double getCost(double[][] expected) {
        ...
        double res = outputStrategy.getCost(a, y);
